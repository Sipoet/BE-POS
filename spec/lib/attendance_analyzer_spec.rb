...
RSpec.describe AttendanceAnalyzer, type: :model do
  ...
  describe '#scheduled_work?' do
    ...
    context 'trade work day' do
      before :each do
        EmployeeDayOff.create!(employee: employee, day_of_week: 7, active_week: :all_week)
        EmployeeLeave.create!(employee: employee, leave_type: :change_day, date: Date.new(2025, 9, 29),
                              change_date: Date.new(2025, 9, 28), change_shift: 1)
        @analyzer = AttendanceAnalyzer.new(
          payroll: employee.payroll,
          employee: employee,
          start_date: start_date,
  ...
  def create_schedule(role)
    (1..7).each do |dow|
      RoleWorkSchedule.create!(group_name: 'normal', begin_work: '08:00', end_work: '15:00', shift: 1, day_of_week: dow,
                               begin_active_at: start_date, role: role, end_active_at: RoleWorkSchedule::LAST_END_DATE)
      RoleWorkSchedule.create!(group_name: 'normal', begin_work: '15:00', end_work: '22:00', shift: 2, day_of_week: dow,
                               begin_active_at: start_date, role: role, end_active_at: RoleWorkSchedule::LAST_END_DATE)
    end
  end
