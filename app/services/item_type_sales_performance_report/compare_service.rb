class ItemTypeSalesPerformanceReport::CompareService < ApplicationService
  def execute_service
    extract_params
    extract_date_range
  ...
  def group_by_item_type(reports, identifier_list)
    brands = Ipos::ItemType.where(name: @item_type_names).index_by(&:name)
    reports.group_by { |row| [row.item_type_name, row.last_purchase_year].compact }
           .map do |keys, values|
             ...
             row.item_type_description = brands[row.item_type_name]&.description
             row
           end
  end
  ...
  class QueryResult
    ...
    def initialize(item_type_name:, value:, date_pk:, last_purchase_year: nil)
  ...
  class LineResult
    ...
    def initialize(item_type_name:, last_purchase_year: nil, spots: [])
      @item_type_name = item_type_name
      @last_purchase_year = last_purchase_year
      @spots = spots
    end
  ...
  def extract_params
    ...
    return unless @item_type_names.empty?

    raise 'Item Type must not Empty'
  end
end
