class Payslip::ReportService < ApplicationService
  ...
  def json_response(results)
    ...
    render_json(PayslipReportSerializer.new(results, include: @included, meta: meta,
                                                     params: { include: @included, payroll_types: @payroll_types }))
  end
  ...
  def excel_response(results)
    ...
    generator.add_metadata({ start_date: @start_date, end_date: @end_date,
                             employee_ids: @employee_ids.try(:join, ',') })
  ...
  def find_response_type
    ...
    response_types.each do |response_type|
      ...
      return 'excel' if ['application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
                         'application/vnd.ms-excel'].include?(response_type.strip)
    end
  end
  ...
  def extract_params
    ...
    raise ExpectedError.new('tanggal mulai dan tanggal akhir harus dipilih') if @start_date.blank? || @end_date.blank?
  ...
  def find_payslips
    ...
    query = query.where('employees.status = ?', @employee_status) if @employee_status.present?
    query = query.where(status: @payslip_status) if @payslip_status.present?
    query
  end
