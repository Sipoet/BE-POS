class Transfer::CreateService < ApplicationService
  ...
  def valid_transfer?(transfer)
    transfer.transfer_items.each do |transfer_item|
      return false if !enough_stock?(transfer_item)
    end
    return true
  end

  def enough_stock?(transfer_item)
    stock_left = Ipos::ItemInInternal.where(kodeitem: transfer_item.kodeitem, kodekantor: transfer_item.kantordari).sum(:sisa)
  ...
  def build_transfer_items(transfer)
    permitted_params = params.required(:data)
                              ...
                              .permit(data:[:type,:id, attributes:[
                                                          :kodeitem,
                                                          :jumlah,
                                                          :satuan,
                                                          :jmlkonversi,
  ...
  def update_internal_table!(transfer_item, transfer)
    ...
    item_in_internals.each do |item_in|
      ...
      item_in_internal = create_item_internal!(item_in: item_in,transfer_item: transfer_item, transfer: transfer, qty: stock)
  ...
  def create_code_for(trid, location: 'GDG')
    ...
    4.times do |index|
      ...
      slot << code_formatter.send("sep#{index + 1}") rescue nil
    end
