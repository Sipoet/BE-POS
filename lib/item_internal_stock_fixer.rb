class ItemInternalStockFixer
  MINUS_TYPE = %w[keluar remasuk remasuk_konsi keluar_konsi transfer]
  ...
  def run!(location)
    ...
    item_transfer_details.each do |transfer_item|
      ...
      if @item.satuan != transfer_item.satuan
        item_uom_qty = Ipos::ItemMeasurementQuantity.find_by(satuan: transfer_item.satuan,
                                                             kodeitem: transfer_item.kodeitem)
        qty *= item_uom_qty.jumlahkonv
      end
      transactions << [transfer_item.transfer.tanggal, 'transfer', qty]
    end
    item_return_details = Ipos::PurchaseItem.where(item: @item,
                                                   purchase: Ipos::ItemInHeader.where(kantortujuan: location))
                                            .includes(:purchase)
    ...
    insert_order_funct = if @item.hppsys == '1'
                           ->((date, key, qty)) { insert_by_fifo(qty, key, item_in_internals, date) }
                         else
                           ->((date, key, qty)) { insert_by_lifo(qty, key, item_in_internals, date) }
                         end
